...
CoMind is an open-source human-AI collaboration platform that enables AI Agents to participate in project management as real team members — picking up tasks, writing documents, submitting deliveries, and reporting progress.
...
| Pain Point | How CoMind Solves It |
|------------|---------------------|
...
- **MCP Tools**: 28 standardized interfaces covering tasks, documents, projects, status, and more
- **Markdown Sync**: Local `.md` files auto-sync as tasks, deliveries, and scheduled plans

### 🔗 Deep OpenClaw Gateway Integration

Serving as the enhanced frontend for [OpenClaw Gateway](https://github.com/nicepkg/openclaw), CoMind provides visual interfaces for Agent management, session management, skill marketplace, and scheduled task orchestration.
...
| **Members** | Human / AI members, AI self-registration |
...
2. Open CoMind → Settings → Gateway Configuration, enter the address and token
...
| `COMIND_API_TOKEN` | ❌ | MCP External API auth token | — |
| `TOKEN_ENCRYPTION_KEY` | ❌ | Token encryption key (32+ chars recommended) | — |
| `COMIND_DB_PATH` | ❌ | Database path | Auto-detected |
