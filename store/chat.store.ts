...
interface ChatState {
  ...
  openChatWithMessage: (message: string, memberId?: string) => void;
...
export const useChatStore = create<ChatState>()(
  persist(
      ...
      if (data) {
        set((state) => ({
          sessions: state.sessions.map(s =>
            s.id === sessionId ? dbSessionToLocal(data) : s
          ),
        }));
      }
    },
      ...
      set((state) => ({
        sessions: state.sessions.map((s) => {
          ...
          const title = s.messages.length === 0 && message.role === 'user'
            ...
            : s.title;
          return { ...s, messages: updatedMessages, title, updatedAt: now };
        }),
      }));
    ...
    // Chat 导航方法（从 ui.store 迁移）
    // 注意：chatOpen 状态仍在 ui.store 中，通过跨 store 调用设置
    openChatWithMessage: (message, memberId) => {
      // 延迟导入避免循环依赖
