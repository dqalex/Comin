...
export type SyncType = 'comind:tasks' | 'comind:schedules' | 'comind:deliveries' | 'comind:milestones' | 'task_list';
...
export function parseFrontmatter(content: string): { frontmatter: Frontmatter | null; body: string } {
  ...
  const isComindType = fm.type && (fm.type.startsWith('comind:') || fm.type === 'task_list');
  const hasDeliveryFields = !!fm.delivery_status;

  // 非 comind 类型且无交付字段 → 不需要解析
