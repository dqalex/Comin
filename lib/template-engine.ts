...
export async function getSystemContext(): Promise<SystemContext> {
  ...
  return {
    ...
    comind_base_url: process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000',
...
/** 列出所有可用模板 */
export function listTemplates(): { name: string; title: string; description: string; comind_version: string }[] {
  ...
  return files.map(f => {
    ...
    return {
      ...
      comind_version: meta.comind_version || process.env.npm_package_version || '2.2.1',
    };
  });
}
...
function parseFrontmatter(content: string): Record<string, string> {
  ...
  const comindVersion = process.env.npm_package_version || '2.2.1';
...
function flattenSystemContext(ctx: SystemContext): Record<string, unknown> {
  return {
    ...
    // 运行时
    comind_base_url: ctx.comind_base_url,
