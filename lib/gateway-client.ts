...
export class OpenClawGatewayClient {
  ...
  /** MCP 配置回调（用于响应 comind.config.request） */
  ...
  /**
   * 设置 MCP 配置回调
   * 当 Gateway 发送 comind.config.request 事件时，调用此回调获取配置并响应
   */
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      // 处理 Gateway 主动请求（如 comind.config.request）
      if (msg.type === 'req' && msg.method === 'comind.config.request' && msg.id) {
  ...
  private async handleConfigRequest(requestId: string): Promise<void> {
    try {
      ...
      gwDebug('Responding to comind.config.request:', config);
  ...
  async listAgents(): Promise<{ defaultId: string; mainKey: string; scope: string; agents: GatewayAgentRow[] }> {
    return this.request<{ defaultId: string; mainKey: string; scope: string; agents: GatewayAgentRow[] }>('agents.list');
  }
  ...
  async listSessions(params?: Record<string, unknown>): Promise<{
    ...
    return this.request('sessions.list', params || { includeGlobal: true, includeUnknown: true, includeDerivedTitles: true, includeLastMessage: true });
  }

  async patchSession(sessionKey: string, updates: Record<string, unknown>): Promise<void> {
    await this.request('sessions.patch', { key: sessionKey, ...updates });
  }
  ...
  async listCronJobs(includeDisabled = true): Promise<{ jobs: CronJob[] }> {
    return this.request<{ jobs: CronJob[] }>('cron.list', { includeDisabled });
  }
  ...
  async createCronJob(job: Record<string, unknown>): Promise<CronJob> {
    return this.request<CronJob>('cron.add', job);
  }
  ...
  async getCronRuns(jobId: string, limit = 30): Promise<CronRunLogEntry[]> {
    const r = await this.request<{ entries: CronRunLogEntry[] }>('cron.runs', { id: jobId, limit });
    return r.entries || [];
  }
  ...
  async sendChatMessage(params: { 
    ...
    await this.request('chat.send', { ...params, idempotencyKey } as Record<string, unknown>);
  }

  async getChatHistory(sessionKey: string, limit = 200): Promise<{ messages: unknown[]; thinkingLevel?: string }> {
    return this.request<{ messages: unknown[]; thinkingLevel?: string }>('chat.history', { sessionKey, limit });
  }

  async abortChat(sessionKey: string, runId?: string): Promise<void> {
    await this.request('chat.abort', runId ? { sessionKey, runId } : { sessionKey });
  }
