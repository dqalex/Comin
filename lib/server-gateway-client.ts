...
export class ServerGatewayClient {
  ...
  private formatActionResults(
    ...
    const lines: string[] = [];
    lines.push(`[CoMind Action Results] (requestId: ${result.requestId})`);
  ...
  private formatUnrecognizedHints(unrecognized: UnrecognizedAction[]): string {
    const lines: string[] = [];
    lines.push('[CoMind Action Parser] 你发送的 actions 中包含无法识别的操作类型:');
    lines.push('');
...
const GLOBAL_CLIENT_KEY = '__comind_server_gateway_client__';
