/**
 * .comind-index 索引文件管理器
 * 
 * 在 workspace 目录下生成并维护 .comind-index 文件，包含：
 * - 心跳状态（供 OpenClaw 判断 CoMind 是否在线）
...
const INDEX_FILE_NAME = '.comind-index';
...
async function writeIndexFile(workspaceId: string): Promise<boolean> {
  try {
    ...
    console.log(`[comind-index] 索引文件已更新: ${indexPath}`);
    return true;
  } catch (error) {
    console.error('[comind-index] 写入索引文件失败:', error);
    return false;
  }
}
...
async function updateHeartbeat(workspaceId: string): Promise<boolean> {
  try {
    ...
    console.error('[comind-index] 心跳更新失败:', error);
    return false;
  }
}
...
const HEARTBEAT_KEY = '__comind_heartbeat_timers__';
...
export function startHeartbeat(workspaceId: string): void {
  ...
  writeIndexFile(workspaceId).catch(err => {
    console.error(`[comind-index] 初始索引写入失败 (${workspaceId}):`, err);
  });
  refreshClaudeMd(workspaceId).catch(err => {
    console.error(`[comind-index] 初始 CLAUDE.md 生成失败 (${workspaceId}):`, err);
  });
  refreshTaskList(workspaceId).catch(err => {
    console.error(`[comind-index] 初始任务列表生成失败 (${workspaceId}):`, err);
  });

  // 定时更新心跳 + 任务列表
  const timer = setInterval(() => {
    updateHeartbeat(workspaceId).catch(err => {
      console.error(`[comind-index] 心跳更新失败 (${workspaceId}):`, err);
    });
    // 任务列表内部有 hash 对比，无变更不会写磁盘
    refreshTaskList(workspaceId).catch(err => {
      console.error(`[comind-index] 任务列表刷新失败 (${workspaceId}):`, err);
    });
  }, intervalMs);

  timers.set(workspaceId, timer);
  console.log(`[comind-index] 心跳已启动 (${workspaceId})，间隔 ${OPENCLAW_CONFIG.heartbeat.interval}s`);
}
...
export function stopHeartbeat(workspaceId: string): void {
  ...
  if (timer) {
    ...
    console.log(`[comind-index] 心跳已停止 (${workspaceId})`);
  }
}
...
export function stopAllHeartbeats(): void {
  ...
  for (const [id, timer] of timers) {
    clearInterval(timer);
    console.log(`[comind-index] 心跳已停止 (${id})`);
  }
  timers.clear();
}
...
export async function startAllHeartbeats(): Promise<number> {
  try {
    ...
    console.error('[comind-index] 启动心跳失败:', error);
    return 0;
  }
}
