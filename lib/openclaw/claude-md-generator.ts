 ...
 * CLAUDE.md 动态生成器
 *
 * 将 CoMind 中的项目列表、成员列表等信息写入 workspace 目录下的 CLAUDE.md，
...
/** 动态数据段的起止标记 */
const DYNAMIC_START = '<!-- COMIND_DYNAMIC_START -->';
const DYNAMIC_END = '<!-- COMIND_DYNAMIC_END -->';
...
function serializeDynamicSection(data: DynamicData): string {
  ...
  lines.push('');
  lines.push(`## 8. CoMind 实时数据`);
  lines.push('');
  lines.push(`> 以下数据由 CoMind 自动生成，更新时间：${data.generatedAt}`);
