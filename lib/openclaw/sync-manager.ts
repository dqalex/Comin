...
export class SyncManager {
   ...
   * 增量同步（单文件）
   * 
   * 当文件是 comind:* 类型时，除了更新 openclawFiles 索引，
  ...
  async syncSingleFile(workspaceId: string, filePath: string, eventType: 'add' | 'change' | 'unlink'): Promise<{
    ...
    if (result.created || result.updated) {
      ...
      // 检测 comind:* 类型文件，触发 Markdown → 任务看板解析
      const comindType = this.detectComindType(frontMatter?.type);
      if (comindType) {
        await this.syncComindFile(workspace, relativePath, content, frontMatter, hash);
      } else {
        // 非 comind:* 类型文件：同步更新关联的 documents 表内容
  ...
  private async syncComindFile(
    ...
    try {
      ...
      const comindType = this.detectComindType(frontMatter?.type);
      const docType = this.mapComindTypeToDocType(comindType || 'note');
      ...
      if (fileRecord?.documentId) {
        ...
        if (syncResult.synced) {
          console.log(`[SyncManager] comind:* 文件解析完成: ${relativePath}`, syncResult.counts);
        ...
        if (syncResult.synced) {
          console.log(`[SyncManager] comind:* 新文件解析完成: ${relativePath}`, syncResult.counts);
          eventBus.emit({ type: 'task_update' });
        }
      }
    } catch (error) {
      console.error(`[SyncManager] comind:* 文件同步失败: ${relativePath}`, error);
    }
  }

  /**
   * 非 comind:* 类型文件变更时，同步更新关联的 documents 表内容
    ...
    try {
      ...
      if (fileRecord.documentId) {
        ...
        await db.update(documents)
          ...
          .where(eq(documents.id, fileRecord.documentId));
        console.log(`[SyncManager] 非 comind 文件 document 更新: ${relativePath}`);
      } else {
        // 无 document → 创建并关联
        const docId = generateId();
        documentId = docId;
        ...
        await db.update(openclawFiles)
          .set({ documentId: docId, updatedAt: new Date() })
          .where(eq(openclawFiles.id, fileRecord.id));
        console.log(`[SyncManager] 非 comind 文件 document 创建: ${relativePath} → ${docId}`);
      }

      // 调用 syncMarkdownToDatabase 处理 delivery_status 等 frontmatter 字段
      // 即使不是 comind:* 类型，只要有 delivery_status 就会自动创建交付记录
      if (documentId) {
        const syncResult = await syncMarkdownToDatabase(documentId, content);
        if (syncResult.synced) {
          console.log(`[SyncManager] 非 comind 文件交付同步: ${relativePath}`, syncResult.type || 'delivery_status');
          eventBus.emit({ type: 'delivery_update' });
        }
      }
    } catch (error) {
      console.error(`[SyncManager] 非 comind 文件同步失败: ${relativePath}`, error);
    }
  }

  /**
   * 检测 comind:* frontmatter 类型
   */
  private detectComindType(type: unknown): string | null {
    if (typeof type !== 'string') return null;
    if (type.startsWith('comind:')) return type;
    if (type === 'task_list') return 'comind:tasks';
    return null;
  }

  /**
   * comind 类型映射到文档类型（与 schema enum 严格匹配）
   */
  private mapComindTypeToDocType(comindType: string): 'guide' | 'reference' | 'report' | 'note' | 'decision' | 'scheduled_task' | 'task_list' | 'other' {
    switch (comindType) {
      case 'comind:tasks': return 'task_list';
      case 'comind:schedules': return 'scheduled_task';
      case 'comind:deliveries': return 'report';
      case 'comind:milestones': return 'note';
      default: return 'note';
    }
  }
