...
export class WorkspaceWatcher {
  ...
  start(workspace: OpenClawWorkspace): boolean {
    ...
    try {
      ...
      const watcher = chokidar.watch(workspace.path, {
        ignored: /node_modules|\.git(\/|$)|\.comind-index|\.comind-pending|tasks\/TODO\.md|tasks\/DONE\.md|\/\.(git|obsidian|clawhub)/,
        persistent: true,
        ignoreInitial: true,
        usePolling: true,
        interval: 100,
  ...
  private buildIgnorePatterns(patterns: string[]): (string | RegExp)[] {
    const defaultPatterns: (string | RegExp)[] = [
      ...
      /\.comind-index/,
      /\.comind-pending/,
      /tasks\/TODO\.md/,         // CoMind 自动生成的任务列表
      /tasks\/DONE\.md/,         // CoMind 自动生成的完成列表
    ];
