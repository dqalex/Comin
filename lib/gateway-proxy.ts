...
export class GatewayProxyClient {
  ...
  async listAgents(): Promise<{ defaultId: string; mainKey: string; scope: string; agents: GatewayAgentRow[] }> {
    return gatewayRequest<{ defaultId: string; mainKey: string; scope: string; agents: GatewayAgentRow[] }>('agents.list');
  }
  ...
  async createAgent(params: { name: string; workspace: string; emoji?: string; avatar?: string }): Promise<{ ok: true; agentId: string; name: string; workspace: string }> {
    return gatewayRequest('agents.create', params as Record<string, unknown>);
  }

  async deleteAgent(agentId: string): Promise<void> {
    await gatewayRequest('agents.delete', { agentId });
  }
  ...
  async listSessions(params?: Record<string, unknown>): Promise<{
    ...
    return gatewayRequest('sessions.list', params || { includeGlobal: true, includeUnknown: true, includeDerivedTitles: true, includeLastMessage: true });
  }
  ...
  async deleteSession(sessionKey: string): Promise<void> {
    await gatewayRequest('sessions.delete', { key: sessionKey });
  }

  async patchSession(sessionKey: string, updates: Record<string, unknown>): Promise<void> {
    await gatewayRequest('sessions.patch', { key: sessionKey, ...updates });
  }

  // ===================== Cron =====================
  async listCronJobs(includeDisabled = true): Promise<{ jobs: CronJob[] }> {
    return gatewayRequest<{ jobs: CronJob[] }>('cron.list', { includeDisabled });
  }
  ...
  async createCronJob(params: Record<string, unknown>): Promise<{ ok: true; id: string }> {
    return gatewayRequest('cron.add', params);
  }
  ...
  async deleteCronJob(id: string): Promise<void> {
    await gatewayRequest('cron.remove', { id });
  }

  async getCronRuns(jobId: string, limit = 50): Promise<CronRunLogEntry[]> {
    const r = await gatewayRequest<{ entries: CronRunLogEntry[] }>('cron.runs', { id: jobId, limit });
    return r.entries || [];
  }
