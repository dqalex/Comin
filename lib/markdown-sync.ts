 ...
 * 支持的文档类型：
 * - comind:tasks      任务看板
 * - comind:schedules  定时任务
 * - comind:deliveries 文档交付
 * - task_list         任务列表（comind:tasks 别名，兼容旧文档）
 * 
...
export async function syncMarkdownToDatabase(documentId: string, content: string): Promise<{ synced: boolean; type?: SyncType; counts?: Record<string, number> }> {
  ...
  // 处理文档 Front Matter 中的交付字段（不限 comind:* 类型，任意 type 均可）
  ...
  switch (frontmatter.type) {
    ...
    case 'comind:schedules':
      return await syncSchedules(documentId, body);
    case 'comind:deliveries':
      return await syncDeliveries(documentId, body);
    case 'comind:milestones':
    {
      // 解析 projectId：与 comind:tasks 相同的逻辑
...
export async function findSyncDocuments(syncType?: SyncType): Promise<{ id: string; type: SyncType }[]> {
  ...
  // 已知的同步类型
  const validTypes: SyncType[] = ['comind:tasks', 'comind:schedules', 'comind:deliveries', 'comind:milestones', 'task_list'];
  
  // 类型别名映射：task_list 是 comind:tasks 的别名
  const typeAliases: Record<string, SyncType[]> = {
    'comind:tasks': ['comind:tasks', 'task_list'],
    'task_list': ['comind:tasks', 'task_list'],
  };
