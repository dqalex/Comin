...
> 本文档是 CoMind 项目的权威编码标准。所有新增、修改代码必须遵循以下规范。
...
- 禁止手动修改 `data/comind.db`，一切变更通过代码驱动
...
- API Token 认证: 环境变量 `COMIND_API_TOKEN` + Bearer Token
- CORS: 同源检测 + `COMIND_CORS_ORIGINS` 白名单
...
3. 在 `comind-mcp.json` 中添加对应的 MCP 工具声明
...
`comind/` 为开发目录，`comind-release/` 为发布目录。每次修改后通过 rsync 同步：

```bash
rsync -av --delete --exclude='node_modules' --exclude='.next' --exclude='.env*' comind/ comind-release/
```
...
**所有部署到生产服务器必须通过 `./.codebuddy/skills/comind/SKILL.md` skill执行，禁止手动 rsync 或其他方式。**
...
- [ ] 是否同步到了 comind-release？
- [ ] 版本号更新时是否同步更新了所有文档？
- [ ] **部署是否通过 `./.codebuddy/skills/comind/SKILL.md` 脚本执行？**
