...
export default function ChatPanel({ onClose }: Props) {
  ...
  const pendingChatMessage = useChatStore((s) => s.pendingChatMessage);
  const pendingChatMemberId = useChatStore((s) => s.pendingChatMemberId);
  const pendingGatewaySessionKey = useChatStore((s) => s.pendingGatewaySessionKey);
  const activeGwSessionKey = useChatStore((s) => s.activeGwSessionKey);
  ...
  useEffect(() => {
    ...
    if (gwConnected && !agentsMainKey) return; // 等待 agentsMainKey
    ...
    if (gwConnected && agentsMainKey) {
      setGwSessionKey(agentsMainKey);
      setActiveGwSessionKey(agentsMainKey);
      ...
      // 加载历史 → 发送消息 → 订阅流式回复
      gwClient.getChatHistory(agentsMainKey, 200)
        .then((result) => {
          ...
          const stream = subscribeChatStream(agentsMainKey, {
  ...
  useEffect(() => {
    if (!pendingGatewaySessionKey || !gwConnected) return;
  ...
  }, [pendingGatewaySessionKey, gwConnected, clearPendingChat, setActiveSession]);

  // 持续监听 Gateway session 的 chat 事件（接收 AI 主动推送）
  useEffect(() => {
    if (!gwSessionKey || !gwConnected) return;

    const stream = subscribeChatStream(gwSessionKey, {
      onDelta: (accumulated) => {
        if (handlerActiveRef.current) return; // 发送中的 handler 优先
        updateGwAssistant(accumulated);
      },
      onComplete: (content) => {
        if (handlerActiveRef.current) return;
        updateGwAssistant(content);
      },
    });

    return () => stream.cleanup();
  }, [gwSessionKey, gwConnected, updateGwAssistant]);
  ...
  const handleGwSessionSend = useCallback((content: string) => {
    ...
    gwClient.sendChatMessage({
      ...
      stream.cleanup();
      setSending(false);
      setErrorMessage('发送消息失败，请检查连接后重试');
    ...
    try {
      ...
      if (gwClient) {
        ...
        if (!sessionKey) {
          ...
          sessionKey = agentId ? `agent:${agentId}` : (agentsMainKey || `comind-${sessionId}`);
        }
  ...
  const handleNewSession = useCallback(async () => {
    ...
    if (gwConnected && agentsMainKey) {
      const newSessionKey = `${agentsMainKey}:comind-${Date.now()}`;
  ...
  }, [selectedMemberId, aiMembers, members, gwConnected, agentsMainKey, createSession, setActiveGwSessionKey]);
        ...
        <div className="flex items-center justify-between px-4 h-14 border-b" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>Gateway 会话</div>
