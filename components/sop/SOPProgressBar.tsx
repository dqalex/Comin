...
// 阶段状态颜色映射
const STAGE_STATUS_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  pending: { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-500', dot: 'bg-slate-400' },
  active: { bg: 'bg-blue-50 dark:bg-blue-950', text: 'text-blue-600 dark:text-blue-400', dot: 'bg-blue-500' },
  waiting_input: { bg: 'bg-amber-50 dark:bg-amber-950', text: 'text-amber-600 dark:text-amber-400', dot: 'bg-amber-500' },
  waiting_confirm: { bg: 'bg-purple-50 dark:bg-purple-950', text: 'text-purple-600 dark:text-purple-400', dot: 'bg-purple-500' },
  completed: { bg: 'bg-emerald-50 dark:bg-emerald-950', text: 'text-emerald-600 dark:text-emerald-400', dot: 'bg-emerald-500' },
  skipped: { bg: 'bg-slate-50 dark:bg-slate-900', text: 'text-slate-400', dot: 'bg-slate-300' },
  failed: { bg: 'bg-red-50 dark:bg-red-950', text: 'text-red-600 dark:text-red-400', dot: 'bg-red-500' },
};

// 阶段状态 i18n key
const STAGE_STATUS_KEYS: Record<string, string> = {
  pending: 'sop.stagePending',
  active: 'sop.stageActive',
  waiting_input: 'sop.stageWaitingInput',
  waiting_confirm: 'sop.stageWaitingConfirm',
  completed: 'sop.stageCompleted',
  skipped: 'sop.stageSkipped',
  failed: 'sop.stageFailed',
};
...
interface SOPProgressBarProps {
  ...
  onStageAction?: (action: 'confirm' | 'reject' | 'skip' | 'start', sopInputs?: Record<string, string>) => Promise<void>;
 ...
 */
export default function SOPProgressBar({
  stages,
  stageHistory,
  currentStageId,
  compact = false,
      ...
      <div className="mt-2 ml-6">
        ...
        {currentStage && currentRecord && (
          <div className="flex items-center gap-1 mt-1">
            <span
              className={clsx('w-1.5 h-1.5 rounded-full flex-shrink-0', STAGE_STATUS_COLORS[currentRecord.status]?.dot || 'bg-slate-400')}
            />
            <span className="text-[10px] truncate" style={{ color: 'var(--text-tertiary)' }}>
              {currentStage.label}
              {' · '}
              {t(STAGE_STATUS_KEYS[currentRecord.status] || 'sop.stagePending')}
            </span>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-1">
          {stages.map((stage, index) => {
            ...
            const colors = STAGE_STATUS_COLORS[status] || STAGE_STATUS_COLORS.pending;
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isCurrent && record && (
                    <span className={clsx('text-[10px]', colors.text)}>
                      {t(STAGE_STATUS_KEYS[record.status] || 'sop.stagePending')}
                    </span>
                  )}
                  {/* 已完成阶段的产出摘要 */}
