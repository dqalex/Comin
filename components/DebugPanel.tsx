...
import SOPDebugPanel from '@/components/sop/SOPDebugPanel';
...
export default function DebugPanel() {
  ...
  // 创建默认用户
  const handleSeedDefaultMember = async () => {
    addLog('Seeding default member...');
    try {
      const res = await fetch('/api/debug', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'seed_default_member' }),
      });
      const data = await res.json();
      addLog(`Seed result: ${data.status} - ${data.message || data.error}`);
      await handleDiagnostics();
    } catch (e) {
      addLog(`Seed ERROR: ${e instanceof Error ? e.message : String(e)}`);
    }
  };
  ...
  const getLocalStorageInfo = (): string[] => {
    const keys = ['comind-ui', 'comind-project-selection', 'comind-chat'];
    return keys.map(key => {
                  ...
                  <div className="mt-3 p-2 rounded-lg border border-red-500/30" style={{ background: 'var(--surface-hover)' }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-semibold text-red-500">
                        发现 {diagnostics.diagnostics.missingTables!.length} 个缺失表
                      </span>
                    </div>
                ...
                {diagnostics.diagnostics.members?.count === 0 && !(diagnostics.diagnostics.missingTables?.includes('members')) && (
                  <div className="mt-2">
                    <ActionButton
                      ...
                      label="创建默认用户"
                      onClick={handleSeedDefaultMember}
                      color="green"
                    />
                  </div>
                )}
