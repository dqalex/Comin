...
export default function DeliveryStatusCard({ document }: DeliveryStatusCardProps) {
  ...
  const { deliveries, updateDeliveryAsync } = useDeliveryStore();
  const { getHumanMembers, members } = useMemberStore();
  const { tasks } = useTaskStore();
  const { projects } = useProjectStore();
  const { agentsMainKey } = useGatewayStore();
  ...
  const handleReview = useCallback(async () => {
    ...
    try {
      ...
      }, agentsMainKey ? { _gatewaySessionKey: agentsMainKey } : undefined);
