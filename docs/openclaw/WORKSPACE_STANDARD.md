...
```
https://teamclaw.app/doc/{id}
https://teamclaw.app/task/{id}
```
...
~/.openclaw/workspace/
├── CLAUDE.md              # 约束文件
├── .teamclaw-index          # 索引（TeamClaw 维护）
├── .teamclaw-pending        # 离线缓存
├── documents/
│   ├── reports/
│   ├── notes/
│   └── task-outputs/
