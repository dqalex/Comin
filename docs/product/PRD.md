# comind-v2 产品需求文档 (PRD)

## 1. 产品定位

comind-v2 是新一代 AI Agent 管理平台，定位为 **OpenClaw Gateway 的增强型前端**。

- **保留 CoMind 核心能力**：任务看板、项目管理、Wiki、聊天、文档交付
...
| 状态管理 | Zustand | 14 个 Store |
| 样式 | Tailwind CSS 3.4 | 深色/浅色主题切换 |
| UI 组件 | shadcn/ui | 15 个基础组件 |
| 图标 | lucide-react | 统一图标库 |
| 数据库 | SQLite + Drizzle ORM | 本地数据存储，21 张表 |
...
> 复用 OpenClaw Cron 底层能力，替代 CoMind 的 Agent 转发方案
...
> 融合 CoMind 成员管理 + OpenClaw Agent/Snapshot 能力
...
37 个 MCP 工具供 AI Agent 调用：

| 工具 | 功能 |
|------|------|
| `get_task` / `list_my_tasks` | 获取任务详情/列表 |
| `update_task_status` | 更新任务状态 |
| `add_task_comment` | 添加任务评论 |
| `create_check_item` / `complete_check_item` | 检查项管理 |
| `get_project` / `get_project_members` | 项目信息 |
| `get_document` / `create_document` / `update_document` / `search_documents` | Wiki 文档 CRUD |
| `update_status` / `set_queue` / `set_do_not_disturb` | AI 状态面板 |
| `create_schedule` / `list_schedules` / `update_schedule` / `delete_schedule` | 定时任务管理 |
| `deliver_document` / `review_delivery` / `list_my_deliveries` / `get_delivery` | 文档交付 |
| `create_milestone` / `list_milestones` / `update_milestone` / `delete_milestone` | 里程碑管理 |
| `register_member` | AI 成员自注册（幂等） |
| `get_template` / `list_templates` | 模板系统 |
| `advance_sop_stage` / `request_sop_confirm` / `get_sop_context` | SOP 阶段推进/确认/上下文 |
| `save_stage_output` / `update_knowledge` | SOP 产出保存/知识库更新 |
| `create_sop_template` / `update_sop_template` | SOP 模板创建/更新 |
| `create_render_template` / `update_render_template` | 渲染模板创建/更新 |
...
## 10. 与 CoMind v1 / OpenClaw Dashboard 对比

| 功能 | CoMind v1 | OpenClaw Dashboard | comind-v2 |
|------|-----------|-------------------|-----------|
...
- [x] Phase 5.5: 全平台国际化 (i18n) 支持 — **v2.0.1**
- [x] Phase 6: Markdown 双向同步、模板引擎迁移 — **v2.0.1**
- [x] Phase 7: 多 Agent 模式（由 OpenClaw Gateway 原生支持，取代 Cowork 角色）
- [x] Phase 8: 文档双向链接 + 图谱可视化（双链自动持久化 + 知识图谱面板已完成）
- [x] Phase 8.5: OpenClaw Workspace 双向同步 + 冲突解决 — **v2.1.0**
- [x] Phase 9: Gateway 双模式（server_proxy / browser_direct）+ 安全加固 — **v2.2.0**
...
- [ ] Phase 13: 多用户支持（NextAuth.js 认证、数据隔离 userId/teamId、权限模型 admin/member/viewer）
