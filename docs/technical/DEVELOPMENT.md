...
TeamClaw V3 是 **AI Agent 管理平台**，作为 **OpenClaw Gateway 的增强型前端**，提供完整的人机协作能力。
...
│   ├── users/page.tsx            # 用户管理（仅管理员）
│   ├── init/page.tsx             # 初始化页面（首次启动引导）
│   ├── blog/page.tsx             # 博客页面
│   ├── skills-management/page.tsx # Skill 管理（v3.0）
│   ├── approvals/page.tsx        # 审批系统（v3.0）
│   └── api/                      # 92 个 REST API 路由（详见 API.md）
│
├── components/                   # 82 个 UI 组件
│   ├── AppShell.tsx              # 应用外壳
...
│   ├── chat/                     # 9 个聊天子组件
│   │   ├── ChatPanel.tsx         # 聊天面板（Gateway+本地双模式）
...
│   ├── skills/                   # Skill 管理组件（v3.0）
│   ├── approvals/                # 审批系统组件（v3.0）
│   ├── openclaw/                 # 2 个 OpenClaw 组件
│   ├── settings/                 # 1 个设置组件
│   └── ui/                       # 14 个 shadcn/ui 基础组件
│
├── store/                        # 18 个 Zustand Store + index.ts
│   ├── index.ts                  # 聚合导出 + useDataInitializer
│   ├── auth.store.ts             # v3.0: 用户认证（登录/登出/当前用户）
│   ├── task.store.ts
│   ├── project.store.ts
│   ├── member.store.ts
│   ├── document.store.ts
...
│   ├── user-mcp-token.store.ts   # v3.0: 用户 MCP Token
│   ├── gateway.store.ts          # Gateway 连接/快照/Agent/Session/Skill/Cron
│   ├── skill.store.ts            # v3.0: Skill 管理
│   ├── approval.store.ts         # v3.0: 审批系统
│   └── ui.store.ts
│
├── lib/                          # 核心库（60 个文件）
...
│   ├── icon-render.ts            # Lucide 图标渲染
│   ├── slot-sync.ts              # MD↔HTML 槽位同步
│   ├── skill-validator.ts        # Skill 结构验证（v3.0）
...
│
├── db/                           # 数据库层
│   ├── schema.ts                 # Drizzle Schema（33 张表）
...
| 项目 | 数量 |
|------|------|
| 页面路由 | **19**（v3.0 新增 /init, /skills-management, /approvals） |
| API 路由目录 | **92**（v3.0 新增认证/用户管理/Skill/审批 API） |
| Zustand Store | **18** + index（v3.0 新增 auth/skill/approval） |
| UI 组件 | **82** |
| Lib 文件 | **60** |
| 数据库表 | **33** |
| MCP 工具 | **37** |
...
### 5.5 Lucide 图标库 `lib/icon-render.ts`
...
**文件**：`db/schema.ts`（33 张表）+ `db/index.ts`（连接管理）
...
`db/index.ts` 检测缺失的表/列并增量添加，无需手动编辑数据库文件。
...
| 敏感字段脱敏 | 敏感字段（如 openclawApiToken）响应前必须经 `@/lib/sanitize` 脱敏 |
