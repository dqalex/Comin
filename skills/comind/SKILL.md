...
| Front Matter 字段 | 同步目标 | 说明 |
|-------------------|---------|------|
| `type: comind:tasks` / `task_list` | tasks 表 | 批量创建/更新任务（文档 type 必须为此值） |
| `type: comind:schedules` | schedules 表 | 创建定时任务（文档 type 必须为此值） |
| `type: comind:deliveries` | deliveries 表 | 批量交付列表（文档 type 必须为此值） |
| `type: comind:milestones` | milestones 表 | 批量创建/更新里程碑（需 project 字段指定项目） |
| `delivery_status: pending` | deliveries 表 | **任意 type 文档**只要有此字段即自动创建交付记录 |
...
| 里程碑行 | milestones | 按标题匹配，自动创建/更新（需 project 字段） |
...
> **交付说明**：文档 Front Matter 中存在 `delivery_status` 字段时，**不论文档 `type` 是什么**（`report`、`decision`、`note` 等均可），系统自动创建/更新交付记录。
> - `pending`：进入交付中心待审核队列
> - `approved`：已通过审核
> - `rejected`：已驳回
> - `revision_needed`：需要修改
> - 注意：`type: comind:deliveries` 是**批量交付列表**的专用类型，与单文档的 `delivery_status` 是两种不同的交付方式
...
| 模板 | 用途 | 验证方式 |
|------|------|---------|
| `task-board.md` | 批量创建/更新任务+里程碑 | `list_my_tasks` + `list_milestones` 确认 |
...
| `deliver_document` | title, platform | document_id, external_url, task_id, description | 提交文档交付 |
...
// 文档类型
doc_type: 'guide' | 'reference' | 'note' | 'report' | 'decision' | 'scheduled_task' | 'task_list' | 'other'
...
| 优先级 | high, medium, low |
| 里程碑状态 | open, in_progress, completed, cancelled |
| AI 状态 | idle, working, waiting, offline |
| 文档类型 | guide, reference, note, report, decision, scheduled_task, task_list, other |
| 同步文档类型 | comind:tasks, comind:schedules, comind:deliveries, comind:milestones, task_list |
