...
import {
  ...
  Shield, ShieldAlert, AlertTriangle, FolderSync, Plus, Folder, Bug,
} from 'lucide-react';
...
type SettingsTab = 'general' | 'openclaw' | 'security' | 'debug' | 'about';

/** SSRF 安全配置存储 key */
const SSRF_CONFIG_KEY = 'comind-ssrf-config';
...
const translations = {
  en: {
    title: 'Settings',
    tabs: { general: 'General', openclaw: 'OpenClaw Settings', security: 'Security', debug: 'Debug', about: 'About' },
    ...
    about: {
      title: `CoMind v${process.env.NEXT_PUBLIC_APP_VERSION || '2.2.0'}`,
  ...
  zh: {
    title: '设置',
    tabs: { general: '通用', openclaw: 'OpenClaw 设置', security: '安全设置', debug: '调试工具', about: '关于' },
...
export default function SettingsPage() {
  ...
  const { workspaces, fetchWorkspaces, createWorkspace } = useOpenClawWorkspaceStore();
  const searchParams = useSearchParams();
  const initialTab = (searchParams.get('tab') as SettingsTab) || 'general';
  const [activeTab, setActiveTab] = useState<SettingsTab>(initialTab);
  const [lang, setLang] = useState(() => typeof window !== 'undefined' ? localStorage.getItem('comind-language') || 'zh' : 'zh');
