...
import {
  ...
  Bot, RefreshCw, Clock, ChevronDown, FolderSync, Home,
} from 'lucide-react';
import clsx from 'clsx';
import { Button, Input, Textarea, Select, Card } from '@/components/ui';

const PATROL_CRON_PREFIX = 'comind-patrol:';

function buildPatrolMessage(projectName: string, projectId: string): string {
  return `你是 CoMind 协作平台的 AI 成员，执行项目「${projectName}」的定时巡检。
...
export default function ProjectsPage() {
  ...
  const { projects, createProject, updateProjectAsync, deleteProjectAsync, setCurrentProject } = useProjectStore();
  const { tasks } = useTaskStore();
  const { documents } = useDocumentStore();
  const {
    connected, connectionMode, serverProxyConnected, cronJobs, agentsList,
    createCronJob, updateCronJob, deleteCronJob, toggleCronJob, refreshCronJobs,
  } = useGatewayStore();
  ...
  const saveEdit = async () => {
    if (submitByEnterRef.current) return;
    submitByEnterRef.current = true;
    if (editingId && editName.trim()) {
      await updateProjectAsync(editingId, { name: editName.trim(), description: editDesc.trim() || undefined });
    }
    setEditingId(null);
  ...
  // ===================== 自动巡检 =====================
  const handleTogglePatrol = useCallback(async (projectId: string, projectName: string) => {
  ...
  const getIntervalLabel = (ms: number) => {
    const opt = INTERVAL_OPTIONS.find(o => o.ms === ms);
    if (opt) return opt.label;
    const hours = ms / (60 * 60 * 1000);
    return hours >= 1 ? `${hours}h` : `${ms / 60000}m`;
  };
