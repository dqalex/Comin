...
import {
  ...
  AlertCircle, Wifi, WifiOff, Loader2, Cpu, HardDrive, Activity,
  Zap, MessageSquare, Monitor, Shield, Radio, Heart, Database,
  RefreshCw, ChevronDown, CheckCircle, XCircle, ArrowUpRight,
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { members, getAIMembers, getHumanMembers } = useMemberStore();
  const { documents } = useDocumentStore();
  const { deliveries } = useDeliveryStore();
  const { statusList, getByMemberId } = useOpenClawStatusStore();
  const {
    connected, connecting, connectionMode, serverProxyConnected,
    error: gwError, connect, disconnect,
    snapshot, health, sessions, sessionsCount, cronJobs, agentsList, agentHealthList,
    gwUrl, gwToken, refreshSnapshot, refreshHealth, skills,
    helloPayload, lastChannelsRefresh,
  } = useGatewayStore();
  ...
  const [editUrl, setEditUrl] = useState(gwUrl || DEFAULT_GW_URL);
  const [editToken, setEditToken] = useState(gwToken || '');
  ...
  const handleConnect = async () => {
    ...
    await connect(editUrl.trim(), editToken.trim());
  };
              ...
              {isGwConnected && (
                <div className="flex items-center gap-2">
                  ...
                  {connectionMode !== 'server_proxy' && (
                    <Button variant="ghost" size="sm" onClick={disconnect}>{t('dashboard.disconnectBtn')}</Button>
                  )}
                </div>
              )}
            </div>

            {!isGwConnected && connectionMode !== 'server_proxy' && (
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="text-[11px] font-semibold block mb-1.5" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.address')}</label>
                  <Input value={editUrl} onChange={e => setEditUrl(e.target.value)} placeholder="ws://localhost:18789" />
                </div>
                <div className="w-48">
                  <label className="text-[11px] font-semibold block mb-1.5" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.tokenOptional')}</label>
                  <Input type="password" value={editToken} onChange={e => setEditToken(e.target.value)} placeholder={t('dashboard.authToken')} />
                </div>
                <Button onClick={handleConnect} disabled={connecting}>
                  {connecting ? <Spinner size="sm" /> : <Wifi className="w-3.5 h-3.5" />}
                  {connecting ? t('dashboard.connecting') : t('dashboard.connectBtn')}
                </Button>
              </div>
            )}
