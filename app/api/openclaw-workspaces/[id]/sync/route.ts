...
async function performSync(
  ...
  for (const file of files) {
    try {
      ...
      const comindType = detectComindType(file.frontMatter?.type);
      ...
      // 检测文档类型
      const fileType = comindType || detectFileType(relativePath);
      ...
      // 处理 comind:* 类型文件 - 调用 markdown-sync 引擎
      if (comindType) {
        // 创建或更新 document 记录
        if (existing) {
          ...
          if (hashUnchanged && !forceReparse) {
            ...
            if (existing.documentId && comindType === 'comind:tasks') {
              ...
              if (relatedTasks.length === 0 && relatedTasksLike.length === 0) {
                needReparse = true;
                console.log(`[sync] comind:* 文件 hash 未变但无关联任务，强制重新解析: ${relativePath}`);
              }
            }
      ...
      if (existing) {
        ...
        if (existing.documentId) {
          ...
          // 处理 delivery_status 等 frontmatter 字段（非 comind 文件也可能有交付字段）
          const syncResult = await syncMarkdownToDatabase(existing.documentId, file.content);
...
/**
 * 检测 comind:* frontmatter 类型
 * task_list 作为 comind:tasks 的别名，同样触发任务同步
 */
function detectComindType(type: unknown): SyncType | null {
  if (typeof type !== 'string') return null;
  // task_list 是 comind:tasks 的别名，返回标准类型
  if (type === 'task_list') return 'comind:tasks';
  if (type === 'comind:tasks' || type === 'comind:schedules' || type === 'comind:deliveries') {
...
function mapFileTypeToDocType(fileType: string): 'guide' | 'reference' | 'report' | 'note' | 'decision' | 'scheduled_task' | 'task_list' | 'other' {
  const mapping: Record<string, 'guide' | 'reference' | 'report' | 'note' | 'decision' | 'scheduled_task' | 'task_list' | 'other'> = {
    ...
    guide: 'guide',
    reference: 'reference',
    'comind:tasks': 'task_list',
    'comind:schedules': 'scheduled_task',
    'comind:deliveries': 'other',
  };
