...
export async function POST(request: NextRequest) {
  try {
    ...
    // 初始化 .comind-index 索引文件 + CLAUDE.md 动态数据
    try {
...
// HEARTBEAT.md 中 CoMind 自动管理段的标记
const COMIND_HEARTBEAT_START = '<!-- COMIND_HEARTBEAT_START -->';
const COMIND_HEARTBEAT_END = '<!-- COMIND_HEARTBEAT_END -->';

/**
 * 在 workspace 目录下生成心跳任务模板文件和 HEARTBEAT.md
 * - tasks/heartbeat-check-progress.md
 * - tasks/heartbeat-daily-report.md
 * - tasks/heartbeat-sync-to-comind.md
 * - HEARTBEAT.md（幂等更新：用标记段保护 CoMind 内容，不影响用户自定义部分）
 */
async function generateHeartbeatFiles(workspacePath: string) {
  ...
  // 心跳模板文件内容（精简版，完整版参见 skills/comind/references/）
...
- \`.comind-index\` — 文件索引（路径→ID/hash/version 映射），对比 hash 检测变更
...
3. 读取 \`.comind-index\` 的 files 段，对比本地文件 hash 检测 workspace 变更
...
- \`.comind-index\` — 文件→ID 映射，交付文档时获取 document_id
...
最轻量的心跳任务，同步 Agent 状态到 CoMind 面板。
...
- \`.comind-index\` — 文件索引（路径→ID/hash/version），检测变更时从此获取 document_id
...
4. 读取 \`.comind-index\` 的 files 段对比本地文件 hash 检测变更
5. 有变更时从 \`.comind-index\` 获取 document_id，调用 \`update_document\` 同步
6. 无异常则回复 HEARTBEAT_OK
...
`,
  ...
  // CoMind 自动管理的标记段内容（每次更新时会被替换，用户自定义内容不受影响）
  const heartbeatSection = `${COMIND_HEARTBEAT_START}
## CoMind 自动维护文件

以下文件由 CoMind 自动生成和维护，心跳任务可直接读取：

| 文件 | 用途 | 刷新频率 |
|------|------|----------|
| \`tasks/TODO.md\` | 我的任务清单（待处理 + 进行中） | 心跳间隔（120s），有变更时写入 |
| \`tasks/DONE.md\` | 近 24h 已完成任务 | 心跳间隔（120s），有变更时写入 |
| \`.comind-index\` | 文件索引（路径→ID/hash/version）、心跳状态 | 心跳间隔（120s） |
...
- 状态同步：读取 tasks/heartbeat-sync-to-comind.md 同步数据

---
保持精简。每项检查都消耗 token。
${COMIND_HEARTBEAT_END}`;
  ...
  if (existsSync(heartbeatPath)) {
    ...
    if (existing.includes(COMIND_HEARTBEAT_START) && existing.includes(COMIND_HEARTBEAT_END)) {
      // 已有标记段 → 替换标记段内容，保留用户自定义部分
      const startIdx = existing.indexOf(COMIND_HEARTBEAT_START);
      const endIdx = existing.indexOf(COMIND_HEARTBEAT_END) + COMIND_HEARTBEAT_END.length;
