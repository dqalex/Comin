...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'seed_default_member') {
      ...
      try {
        sqlite.prepare(
          `INSERT OR IGNORE INTO members (id, name, type, email, online, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`
        ).run('member-default', '默认用户', 'human', '', 1, now, now);
        return NextResponse.json({
          status: 'ok',
          message: '默认用户已创建（或已存在）',
        });
      } catch (e) {
        return NextResponse.json({
          status: 'error',
    ...
    if (action === 'test_query') {
      ...
      for (const table of tablesToTest) {
        try {
          const rows = sqlite.prepare(`SELECT COUNT(*) as cnt FROM ${table}`).get() as { cnt: number };
          // 也测试 SELECT * 查询（Drizzle ORM 的模式）
          sqlite.prepare(`SELECT * FROM ${table} LIMIT 1`).get();
          results[table] = { ok: true, count: rows.cnt };
        } catch (e) {
    ...
    return NextResponse.json({
      status: 'error',
      error: `未知操作: ${action}，支持: repair_tables, repair_columns, seed_default_member, test_query`,
    }, { status: 400 });
  } catch (e) {
    return NextResponse.json({
      status: 'error',
