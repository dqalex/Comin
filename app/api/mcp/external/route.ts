...
import { TOOL_HANDLERS, MEMBER_SCOPED_TOOLS, COMIND_VERSION } from '../handlers/tool-registry';
...
async function authenticateMember(request: NextRequest): Promise<{ memberId: string; rawToken: string; error?: never } | { memberId?: never; rawToken?: never; error: string }> {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return { error: '缺少 Authorization 头，格式: Bearer <member_api_token>' };
  }
  ...
  if (cached && Date.now() - cached.cachedAt < TOKEN_CACHE_TTL) {
    if (cached.mode === 'chat_only') {
      return { error: '成员执行模式不允许 API 直调，请切换为 api_first 或 api_only' };
    }
...
async function executeTool(
  ...
  if (!handler) {
    return { success: false, error: `工具 ${tool} 暂未实现` };
  }
  ...
  try {
    ...
    if (body.batch && Array.isArray(body.batch)) {
      if (body.batch.length > 50) {
        return NextResponse.json({ success: false, error: '批量调用数量超过限制（最多 50 个）' }, { status: 400 });
      }
      ...
      for (const call of body.batch) {
        ...
        logMcpCall(call.tool, call.parameters || {}, result, {
          source: 'mcp_external',
          memberId,
          apiToken: rawToken,
          requestId,
    ...
    if (!tool) {
      return NextResponse.json({ success: false, error: '缺少 tool 字段' }, { status: 400 });
    }
    ...
    console.error('[CoMind-v2] POST /api/mcp/external error:', error);
