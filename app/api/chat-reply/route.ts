...
function getSsrfConfig(request: NextRequest): SSRFConfig {
  // 从 cookie 读取用户设置
  const ssrfConfigCookie = request.cookies.get('comind-ssrf-config');
  if (ssrfConfigCookie) {
    try {
...
async function handlePost(request: NextRequest) {
  try {
    ...
    if (!memberId || typeof memberId !== 'string') {
      return NextResponse.json({ error: 'memberId 格式无效' }, { status: 400 });
    }
    ...
    if (member.type !== 'ai') {
      return NextResponse.json({ error: '非 AI 成员' }, { status: 400 });
    }
...
async function handleKnotReply(
  ...
  if (!agentId || !apiToken) {
    return NextResponse.json(
      { error: '该 AI 成员缺少 Agent ID 或 API Token 配置' },
      { status: 400 },
    );
  }
  ...
  if (!response.ok) {
    ...
    let errorMessage = 'Knot 服务请求失败';
    if (response.status === 401) {
      errorMessage = 'Knot API Token 无效或已过期';
    } else if (response.status === 429) {
      errorMessage = '请求频率过高，请稍后重试';
    } else if (response.status >= 500) {
      errorMessage = 'Knot 服务暂时不可用';
    }
  ...
  if (contentType.includes('application/json') || (!text.includes('data:') && text.startsWith('{'))) {
    try {
      ...
      if (json.code && json.code !== 0) {
        return NextResponse.json({ error: json.msg || `Knot 错误 (code: ${json.code})` }, { status: 401 });
      }
...
async function handleOpenClawReply(
  ...
  if (!ssrfResult.allowed) {
    ...
    return NextResponse.json(
      { error: ssrfResult.error || 'endpoint 地址不被允许' },
      { status: 400 }
    );
  }
  ...
  if (!response.ok) {
    ...
    let errorMessage = 'OpenClaw 服务请求失败';
    if (response.status === 401) {
      errorMessage = 'OpenClaw Token 无效';
    } else if (response.status === 429) {
      errorMessage = '请求频率过高，请稍后重试';
    } else if (response.status >= 500) {
      errorMessage = 'OpenClaw 服务暂时不可用';
    }
    return NextResponse.json({ error: errorMessage }, { status: response.status });
  }
