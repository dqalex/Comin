...
export async function GET(
  ...
  try {
    ...
    if (!delivery) {
      return NextResponse.json({ error: '交付记录不存在' }, { status: 404 });
    }

    return NextResponse.json(delivery);
  } catch (error) {
    return NextResponse.json({ error: '获取交付记录失败' }, { status: 500 });
  }
}
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ error: '更新交付记录失败' }, { status: 500 });
  }
}
...
async function buildReviewNotification(
  ...
  try {
      ...
      if (attachedDocs.length > 0) {
        ...
        const fileList = [
          ...
          ...attachedDocs
            ...
            .map(doc => `- **${doc.title}** [CoMind 存储] - doc:${doc.id}`),
        ].join('\n');
    ...
    const finalMessage = message || `[CoMind 审核通知] 你提交的文档「${ctx.deliveryTitle}」审核结果: ${statusLabel}。${ctx.reviewComment ? `\n审核意见: ${ctx.reviewComment}` : ''}\n请根据审核意见修改后重新提交。`;
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: '删除交付记录失败' }, { status: 500 });
  }
}
