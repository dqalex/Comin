...
export async function POST(
  ...
  if (!action || !['confirm', 'reject', 'skip', 'start'].includes(action)) {
    return NextResponse.json({ error: '无效的操作类型，支持: confirm, reject, skip, start' }, { status: 400 });
  }
  ...
  if (!task.sopTemplateId) {
    return NextResponse.json({ error: '该任务未绑定 SOP 模板' }, { status: 400 });
  }
  ...
  if (action === 'start') {
    if (task.currentStageId && stageHistory.some(r => r.status === 'active' || r.status === 'waiting_input' || r.status === 'waiting_confirm')) {
      return NextResponse.json({ error: 'SOP 已在执行中' }, { status: 400 });
    }

    const firstStage = stages[0];
    if (!firstStage) {
      return NextResponse.json({ error: 'SOP 模板无阶段' }, { status: 400 });
    }
  ...
  if (!task.currentStageId) {
    return NextResponse.json({ error: 'SOP 未开始执行，请先调用 start' }, { status: 400 });
  }

  const currentIndex = stages.findIndex(s => s.id === task.currentStageId);
  if (currentIndex === -1) {
    return NextResponse.json({ error: '当前阶段在模板中不存在' }, { status: 400 });
  }
  ...
  if (action === 'reject') {
    ...
    if (!rollbackTargetId) {
      return NextResponse.json({ error: '无法回退：没有可回退的阶段' }, { status: 400 });
    }

    const rollbackIndex = stages.findIndex(s => s.id === rollbackTargetId);
    if (rollbackIndex === -1) {
      return NextResponse.json({ error: '回退目标阶段不存在' }, { status: 400 });
    }
  ...
  if (action === 'skip') {
    if (!currentStage.optional) {
      return NextResponse.json({ error: '该阶段不可跳过' }, { status: 400 });
    }
  ...
  return NextResponse.json({ error: '未知操作' }, { status: 400 });
    ...
    return NextResponse.json({ error: 'SOP 阶段操作失败' }, { status: 500 });
  }
}
