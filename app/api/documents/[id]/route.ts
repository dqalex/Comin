...
import { db, documents, deliveries, tasks, openclawFiles } from '@/db';
import { openclawVersions, openclawConflicts } from '@/db/schema';
...
export async function PUT(
  ...
  try {
    ...
    if (body.content && typeof body.content === 'string' && body.content.length > 10 * 1024 * 1024) {
      return NextResponse.json({ error: '文档内容超过大小限制 (10MB)' }, { status: 413 });
    }
    ...
    if (body.type !== undefined && !validateEnum(body.type, VALID_DOC_TYPE)) {
      return NextResponse.json({ error: `type 必须是 ${VALID_DOC_TYPE.join('/')} 之一` }, { status: 400 });
    }
