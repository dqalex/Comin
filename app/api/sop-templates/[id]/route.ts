...
import { db, sopTemplates, tasks } from '@/db';
...
// 允许 PUT 更新的字段
const allowedFields = [
  'name', 'description', 'category', 'icon', 'status',
...
export async function GET(
  ...
  try {
    ...
    if (!template) {
      return NextResponse.json({ error: '未找到 SOP 模板' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ error: '更新 SOP 模板失败' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (taskRef) {
      return NextResponse.json({ error: '无法删除：该模板正被任务使用' }, { status: 409 });
    }
    ...
    return NextResponse.json({ error: '删除 SOP 模板失败' }, { status: 500 });
  }
}
