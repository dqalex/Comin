...
function buildSetupTasks() {
  ...
  return [
    {
      title: '安装 CoMind Skill',
      description: `## 任务目标
安装最新版本的 CoMind Skill 到 OpenClaw Agent。

## 执行步骤
1. 检查当前已安装的 Skill 版本（所在目录 ${skillDir}）
2. 安装最新版本 CoMind Skill（最新版本是 V${version}）
3. 记录安装信息：
   - Skill 版本号
   - Skill 安装目录
   - 安装时间
...
- 版本信息已记录在任务评论中`,
      ...
      checkItems: [
        ...
        { id: 'check-2', text: '安装最新 CoMind Skill', completed: false },
    ...
    },
  {
    ...
    description: `## 任务目标
根据 CoMind Skill 文档获取 MCP API Token 并完成配置联调。

## 执行步骤
1. 阅读 CoMind Skill 文档中的 MCP 配置说明
2. 选择 Token 获取方式：
   - 方式一（推荐）：通过 WebSocket 发送 \`comind.config.request\` 事件自动获取
   - 方式二：在 CoMind 成员管理页面手动生成 Token
3. 配置 OpenClaw 环境变量：
   - \`COMIND_BASE_URL\`：CoMind 实例地址
   - \`COMIND_API_TOKEN\`：获取到的 Token
4. 测试 MCP 连接

## 完成标准
- MCP Token 配置成功
- 能够成功调用 CoMind MCP 工具`,
    priority: 'high' as const,
    checkItems: [
  ...
  },
  {
    ...
    description: `## 任务目标
测试 CoMind MCP 工具功能并生成测试报告推送到文档交付中心。
...
export async function POST(
  ...
  try {
    ...
    if (member.type !== 'ai') {
      return NextResponse.json({ error: '只有 AI 成员支持一键配置' }, { status: 400 });
    }

    // 检查是否已存在同名项目
    const existingProjects = await db.select().from(projects);
    const existingProject = existingProjects.find(
      p => p.name === '联调 CoMind MCP' || p.name === 'CoMind MCP Integration'
    );
    ...
    if (existingProject) {
      ...
      const newProject: NewProject = {
        ...
        name: '联调 CoMind MCP',
        description: `## 项目说明
本项目用于 OpenClaw Agent 与 CoMind MCP 系统的联调测试。

### 目标
1. 安装并配置 CoMind Skill
2. 获取 MCP API Token 并完成鉴权配置
3. 测试 MCP 工具功能
4. 生成测试报告

### 相关资源
- CoMind 文档：查看 Skill 文档了解 MCP 配置方法
- 成员管理：在 CoMind 成员管理页面管理 API Token`,
        source: 'local',
        createdAt: new Date(),
        updatedAt: new Date(),
      };
