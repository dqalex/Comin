...
import { db, members, openclawStatus, scheduledTasks, scheduledTaskHistory, deliveries } from '@/db';
import { openclawWorkspaces, openclawFiles, openclawVersions, openclawConflicts, chatSessions, chatMessages } from '@/db/schema';
      ...
      if (body[field] !== undefined) {
        ...
        if (enumValidation[field] && body[field] !== null) {
          if (!validateEnum(body[field], enumValidation[field] as readonly string[])) {
            return NextResponse.json({ error: `${field} 值不合法` }, { status: 400 });
          }
        }
        // Token 加密存储
