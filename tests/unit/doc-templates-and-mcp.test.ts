...
import { COMIND_TOOLS } from '@/core/mcp/definitions';
...
describe('DOC_TEMPLATES', () => {
  ...
  it('task_list 模板应该包含 frontmatter', () => {
    ...
    expect(template).toContain('type: comind:tasks');
  });
...
describe('COMIND_TOOLS', () => {
  it('应该导出一个非空对象', () => {
    expect(typeof COMIND_TOOLS).toBe('object');
    expect(Object.keys(COMIND_TOOLS).length).toBeGreaterThan(0);
  });

  it('每个工具应该有 name、description、parameters', () => {
    for (const [key, tool] of Object.entries(COMIND_TOOLS)) {
  ...
  // 关键工具存在性验证
  it('应该包含核心任务工具', () => {
    expect(COMIND_TOOLS).toHaveProperty('get_task');
    expect(COMIND_TOOLS).toHaveProperty('list_my_tasks');
    expect(COMIND_TOOLS).toHaveProperty('update_task_status');
    expect(COMIND_TOOLS).toHaveProperty('add_task_comment');
  });

  it('应该包含文档工具', () => {
    expect(COMIND_TOOLS).toHaveProperty('create_document');
    expect(COMIND_TOOLS).toHaveProperty('update_document');
  });

  it('应该包含成员注册工具', () => {
    expect(COMIND_TOOLS).toHaveProperty('register_member');
  });
});
