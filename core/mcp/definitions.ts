 ...
 * 
 * 18 个工具的完整定义，移植自 comind v1 并针对 v2 优化
 */

export const COMIND_TOOLS = {
  get_task: {
    name: 'get_task',
  ...
  list_my_tasks: {
    name: 'list_my_tasks',
    description: '获取分配给当前 AI 成员的任务列表。通过外部 API 调用时身份自动注入，无需传 member_id。也支持按昵称查询',
    parameters: {
      type: 'object',
      properties: {
        status: { 
  ...
  deliver_document: {
    ...
    parameters: {
      ...
      properties: {
        ...
        document_id: { type: 'string', description: 'CoMind 内部文档 ID' },
...
export type ComindToolName = keyof typeof COMIND_TOOLS;

export function getToolDefinitions() {
  return Object.values(COMIND_TOOLS);
}
